...
import { ChunkWord, SpeakBtn, Mnemonic, Steps } from './ui.jsx';
import {
  prepareImage,
  extractFromImage,
  extractPhonics,
...
/** 상단 단계. 조립·이야기는 "덩어리" 안의 연습이라 같은 칸에 둔다 (칸을 늘리면 폰에서 글자가 깨진다) */
const WORD_STEPS = [
  { key: 'upload', label: '사진' },
  { key: 'phonics', label: '파닉스' },
  { key: 'chunks', label: '덩어리' },
  { key: 'quiz', label: '시험' },
  { key: 'review', label: '오답' },
  { key: 'final', label: '최종' },
];
const STEP_INDEX = { upload: 0, check: 0, phonics: 1, chunks: 2, assemble: 2, story: 2, quiz: 3, review: 4, final: 5, done: 5 };
const stepIndex = (key) => STEP_INDEX[key] ?? 0;

export default function WordLab({ onHome }) {
  ...
  return (
    <div className="wl">
      <div className="wl-shell">
        ...
        {showSteps && <Steps items={WORD_STEPS} idx={stepIndex(shellStep)} onJump={jump} canJump={canJump} />}
